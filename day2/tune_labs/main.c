...
main(int argc, char *argv[]) {
    ...
    if ( (parts.ptype = calloc( nparts, sizeof(*parts.ptype) )) == NULL ) {
	perror("main(__LINE__), allocation failed");
	exit(1);
    }
    ...
    #else
    memory = nparts * (sizeof(*parts.x) * 3 + sizeof(*parts.ptype) 
                       + sizeof(*parts.dist));
    #endif
    ...
    */
    ...
    FILE *fptr;
    #ifdef ARRAY_OF_STRUCTS
        fptr = fopen("AOS_performance.dat","a");
    #else
        fptr = fopen("SOA_performance.dat","a");
    #endif

    fprintf(fptr,"%f,%f,%f,%f,%f",memory,mf_dist,mf_check,mf_main,te_main);
    fclose(fptr);
